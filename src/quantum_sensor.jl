...
QuantumMagnetometer(;bias=SVector{3,Float64}(0.0,0.0,0.0), scale=SVector{3,Float64}(1.0,1.0,1.0), noise_std=SVector{3,Float64}(1.0,1.0,1.0), tau=0.01) =
...
function step_measure!(sens::QuantumMagnetometer, true_vector::SVector{3,Float64}, R::SMatrix{3,3,Float64}, dt::Float64)
    ...
    noise = SVector{3,Float64}(rand(Normal(0.0, sens.noise_std[1])), rand(Normal(0.0, sens.noise_std[2])), rand(Normal(0.0, sens.noise_std[3])))
